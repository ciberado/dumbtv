...
export default async function getShowsEntries() {
  ...
  return {
    trending : trending.filter(s => s?.streamingLinks?.offers?.length > 0),
    action: action.filter(s => s?.streamingLinks?.offers?.length > 0),
    netflix: netflix.filter(s => s?.streamingLinks?.offers?.length > 0),
    topRated: topRated.filter(s => s?.streamingLinks?.offers?.length > 0),
    comedy: comedy.filter(s => s?.streamingLinks?.offers?.length > 0),
    documentary: documentary.filter(s => s?.streamingLinks?.offers?.length > 0)
  };
